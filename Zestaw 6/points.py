...
class TestPoint(unittest.TestCase):
    ...
    def test_eq_points(self):
        ...
        self.assertFalse(Point(0, 0).__eq__(Point(21, 4)))

    def test_ne_points(self):
        self.assertFalse(Point(2, 4).__ne__(Point(2, 4)))
        self.assertTrue(Point(0, 0).__ne__(Point(21, 4)))

    def test_add_points(self):
        self.assertEqual(Point(5, 8).__add__(Point(2, 4)), Point(7, 12))
        self.assertEqual(Point(2, -3).__add__(Point(2, 4)), Point(4, 1))
        self.assertEqual(Point(100, 0).__add__(Point(-100, 100)), Point(0, 100))

    def test_sub_points(self):
        self.assertEqual(Point(5, 8).__sub__(Point(2, 4)), Point(3, 4))
        self.assertEqual(Point(2, -3).__sub__(Point(2, 4)), Point(0, -7))
        self.assertEqual(Point(100, 0).__sub__(Point(-100, 100)), Point(200, -100))

    def test_mul_points(self):
        self.assertEqual(Point(5, 8).__mul__(Point(2, 4)), 42)
        self.assertEqual(Point(2, -3).__mul__(Point(2, 4)), -8)
        self.assertEqual(Point(100, 0).__mul__(Point(-100, 100)), -10000)
